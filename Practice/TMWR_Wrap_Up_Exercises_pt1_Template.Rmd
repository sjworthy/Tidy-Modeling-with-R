...
```{r}
set.seed(707)
hr_split = initial_split(dat, prop = 0.75)
hr_train = training(hr_split)
hr_test = testing(hr_split)
hr_folds <- rsample::vfold_cv(hr_train, v = 5)
```
...
Creating a random grid of size 10 for the lm_pen parameters
...
lm_pen_param
lm_pen_param %>% 
  grid_random(size = 10) %>% # 'size' is the number of combinations
  summary()
```
...
```{r}
show_best(lm_pen_tune) %>% select(-.estimator)
```
...
bayes_param <- 
  both_tune_racing %>%
  extract_parameter_set_dials(id = "simple_rf") %>%
  finalize(hr_train)
bayes_param
```
```{r}
